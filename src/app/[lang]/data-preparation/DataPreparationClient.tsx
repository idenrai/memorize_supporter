        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {templates.map(template => (
            <button
              ...
              className={`relative p-5 rounded-2xl text-left transition-colors duration-200 border focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-indigo-500 ${
                selectedTemplate?.id === template.id
                  ? 'border-indigo-500 bg-indigo-500/10 shadow-lg shadow-indigo-500/20'
                  : 'border-white/10 bg-white/5 hover:border-zinc-600 hover:bg-white/10'
              }`}
            >
              <div className={`font-bold text-base ${selectedTemplate?.id === template.id ? 'text-indigo-400' : 'text-zinc-200'}`}>
                {template.name}
              </div>
      ...
      {selectedTemplate && (
        <div className="relative z-10 mt-8">
          ...
          <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-zinc-950 shadow-inner group">
            <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-indigo-500/50 via-purple-500/50 to-pink-500/50 opacity-50" />
            <textarea
              readOnly
