...
- **핵심 컴포넌트**:
  ...
  - `DeckEmptyState` (`src/components/home/DeckEmptyState.tsx`): AI 안티패턴(거대 네온 박스, 3단 장식 카드 등)을 전면 배제하고, 직관적인 2-컬럼 Quick Start 타일('샘플 덱 체험' + '나만의 덱 만들기')로 간소화된 온보딩을 제공하는 독립 프레젠테이션 컴포넌트
  - `EditDeckModal` (`src/components/management/EditDeckModal.tsx`): 데이터 관리 화면에서 긴 덱 제목이나 시리즈/카테고리를 원본 JSON 재업로드 없이 웹 브라우저에서 직접 수정할 수 있는 접근성 모달 컴포넌트 (WAI-ARIA Focus Trap, Return Focus, ESC 닫기 및 한글/일본어 IME 글자 조합 중 Enter 조기 제출 방어 지원)
  - `DeckPlayer`, `Flashcard`, `VocabularyCard`, `MultipleChoiceQuizCard`: 플래시카드, 단어장, 객관식 퀴즈 카드의 인터랙티브 렌더링 및 모션 피드백을 전담하는 학습 카드 컴포넌트군
  - `CardFeedbackActions` (`src/components/cards/CardFeedbackActions.tsx`): 플래시카드와 단어장 카드의 뒷면 피드백 조작을 일원화한 공통 컴포넌트로, WCAG 44×44px 모바일 터치 규격 충족, 또렷한 SVG 화살표(`size={22}`) 및 Zero Hardcoded 텍스트 정책을 준수
...
- **스타일링, 마이크로 애니메이션 및 접근성 (Precision Canvas)**:
  ...
  - **퀴즈 결과 뷰의 역할 분리(Separation of Concerns) 및 접근성 보강**: 퀴즈 결과 및 해설 화면에서 좌측 컨트롤(체크박스/라디오)은 사용자의 선택 상태(선택 시 체크 아이콘)를 1:1로 보존하고, 카드 배경색 및 테두리는 시스템의 채점 결과(녹색 정답 / 빨간색 선택 오답 / 회색 패스)를 전담하도록 설계되었습니다. 조잡한 괄호 병기(`(내 선택)`, `(오답)`)를 완전히 제거하고 정답 보기에만 `[정답]` 단일 뱃지를 부여하며, 시각 장애인을 위한 스크린 리더 숨김 안내(`<span className="sr-only"> ({t.quiz.yourChoiceBadge})</span>`) 및 놓친 정답 번호의 `font-bold` 시각 대비를 완비했습니다.
  ...
  - **컨텍스트 인지형 시험 모드 UI (Context-Aware Exam Mode UI)**: 홈 대시보드(`DeckCard`, `DeckList`, `SearchAndFilter`)에서 시험 모드 활성화 시 퀴즈 덱에만 동적 라벨("출제 문항 수", "전체 문항", "시험 응시", "클릭하여 시험 시작") 및 강조 인디고 액션 스타일을 적용하고, 비퀴즈 덱(플래시카드, 단어장)은 클릭을 비활성화(`aria-disabled="true"`, 딤드/단일 안내 뱃지)하여 오진입을 원천 차단하며, URL 직진입 시에도 `DeckClientLoader`가 안전하게 일반 학습 모드(`mode="practice"`)로 자동 폴백합니다.
  - **포커스 독립적 전역 키보드 인터랙션 (Global Keyboard Shortcuts via `handlersRef`)**: 플래시카드, 단어장, 객관식 퀴즈 컴포넌트 전반에 걸쳐 `window.addEventListener('keydown')` 기반 전역 리스너를 탑재하여 마우스 클릭 등으로 포커스가 유실되어도 단축키(`Space`/`Enter` 플립, `←`/`→` 판정, `1`~`9` 선택)가 끊김 없이 즉각 작동하며, 최신 핸들러 참조(`handlersRef`) 패턴을 통해 불필요한 이벤트 리스너 해제/재등록 부하를 원천 방지했습니다.
...
- **Core Components**:
  ...
  - `DeckEmptyState` (`src/components/home/DeckEmptyState.tsx`): Presentation component dedicated to initial onboarding, redesigned with a streamlined 2-column Quick Start layout ("Try Sample Decks" + "Create Custom Deck") while stripping legacy AI anti-patterns (giant neon icon containers, 3-step decorative cards).
  - `EditDeckModal` (`src/components/management/EditDeckModal.tsx`): Accessible modal dialog enabling inline editing of deck titles and series/categories directly on the data management page without re-uploading source JSON files. Fully equipped with WAI-ARIA Focus Trap, Return Focus, ESC dismiss, and CJK IME composition guards (`e.nativeEvent.isComposing`).
  ...
  - `CardFeedbackActions` (`src/components/cards/CardFeedbackActions.tsx`): Shared component unifying feedback actions for flashcards and vocabulary cards, complying with WCAG 44×44px mobile touch standards, crisp SVG arrow iconography (`size={22}`), and zero hardcoded UI text policy.
...
- **Styling, Micro-animations, and Accessibility (Precision Canvas)**:
  ...
  - **Quiz Result View Separation of Concerns & Enhanced Accessibility**: In quiz result/review views, the UI cleanly separates concerns: left-side checkbox/radio controls preserve the user's exact selection state (rendering checked indicators for selected choices), while card backgrounds and borders convey evaluation feedback (Emerald for correct, Rose for user-selected incorrect, and neutral for unpicked choices). Eliminates cluttered parenthesized labels in favor of a single `[Correct]` badge on correct choices, backed by screen reader-only text (`sr-only`) and bold contrast typography on missed answers.
  ...
  - **Context-Aware Exam Mode UI**: When exam mode is toggled on the home dashboard (`DeckCard`, `DeckList`, `SearchAndFilter`), dynamic contextual copywriting ("Question Count", "All Questions", "Take Exam", "Click to Start Exam") and indigo button emphasis are selectively rendered only for quiz decks, while non-quiz decks (flashcards, vocabulary) have click interactions disabled (`aria-disabled="true"`, dimmed with dedicated unsupported badge) to prevent invalid entry, with `DeckClientLoader` automatically falling back to practice mode (`mode="practice"`) upon direct URL access.
  - **Focus-Independent Global Keyboard Shortcuts (`handlersRef`)**: Integrates top-level `window.addEventListener('keydown')` across flashcards, vocabulary, and multiple-choice quiz cards, ensuring shortcut controls (`Space`/`Enter` to flip, `←`/`→` to rate, `1`~`9` to pick options) operate seamlessly even when container focus is blurred, utilizing the `handlersRef` pattern to eliminate event listener re-binding overhead.
