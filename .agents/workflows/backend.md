...
- **Next.js & TypeScript 스킬 참조:**
  ...
  - **AI 프롬프트 및 LLM 연동 작업 시:** Server Actions, API Routes 또는 서버 로직에서 LLM API 호출, 프롬프트 동적 조립, 구조화된 응답(JSON/Zod) 파싱을 구현하거나 수정할 때는 반드시 `.agents/workflows/prompt.md` 워크플로우를 참조합니다.
